...
router.post('/', asyncHandler(async (req, res) => {
    try {
        uploadPosters.single('img')(req, res, async function (err) {
            ...
            let imageUrl = 'no_url';
            if (req.file) {
                imageUrl = `https://classy-bubblegum-0e4d00.netlify.app/image/poster/${req.file.filename}`;
            }
...
router.put('/:id', asyncHandler(async (req, res) => {
    try {
        ...
        uploadPosters.single('img')(req, res, async function (err) {
            ...
            if (req.file) {
                image = `https://classy-bubblegum-0e4d00.netlify.app/image/poster/${req.file.filename}`;
            }
            ...
            try {
                const updatedPoster = await Poster.findByIdAndUpdate(posterID, { posterName: posterName,productId:productId, imageUrl: image }, { new: true });
                if (!updatedPoster) {
                    return res.status(404).json({ success: false, message: "Poster not found." });
                }
